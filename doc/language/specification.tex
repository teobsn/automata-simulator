...
\paragraph{Practical Example: String Filtering}
...
\paragraph{Practical Example: Adventure Game}
...
\subsubsection{NFA Transition Syntax}
...
\paragraph{Practical Example: Suffix Matching}
...
\subsubsection{PDA Specific Sections}
...
\subsubsection{PDA Transition Syntax}
...
\paragraph{Practical Example: Balanced Sequences}
...
\begin{itemize}
    \item \textbf{Generation}: produce one or more derivations from the grammar.
    \item \textbf{Verification}: determine whether a given input string can be derived from the grammar.
\end{itemize}
...
\subsubsection{CFG Execution Modes}
\begin{itemize}
    \item \textbf{Generation mode}: run the simulator with \texttt{CFG} and \texttt{--count <N>} without \texttt{--verify}. The emulator generates \texttt{N} derivations and prints each derivation step-by-step.
    \item \textbf{Verification mode}: provide an \texttt{input\_string} together with \texttt{--verify}. The emulator checks whether the grammar can derive that string and reports the result as accepted or rejected.
\end{itemize}

Generated derivations are subject to a safety limit to prevent infinite expansion on recursive grammars. If a derivation is truncated, the output includes a warning so the result is not mistaken for a complete derivation.
The limit can be adjusted from the command line with \texttt{--cfg-max-expansions <N>} in generation mode. Use \texttt{0} to disable the limit entirely.
...
\paragraph{Practical Example: Nested Patterns}
...
\clearpage
\section{Automaton Transformation Tools}
In addition to simulation, the project includes standalone file-to-file tools located in \texttt{tools/}. These utilities read and write the same ASL format used by the simulator, so the generated files can be fed back into the existing emulator without any additional conversion.

\subsection{DFA / NFA Conversion}
The \texttt{tools/convert\_automaton.py} script supports deterministic and non-deterministic representations:
\begin{itemize}
    \item \texttt{--to dfa}: Converts an NFA to an equivalent DFA using subset construction.
    \item \texttt{--to nfa}: Rewrites a DFA as an equivalent NFA-compatible ASL file.
\end{itemize}
...
The \texttt{tools/minimise\_dfa.py} script applies DFA minimisation and writes the minimized automaton back to disk using the same ASL structure.
\begin{lstlisting}
python3 tools/minimise_dfa.py input.dfa output.min.dfa
\end{lstlisting}

\subsection{Automaton Visualization}
The \texttt{tools/visualize\_automaton.py} script renders a diagram-only SVG preview of a DFA, NFA, or PDA using Graphviz \texttt{dot}. The output uses black transitions and labels on a white background for a clean academic presentation. It opens the preview in the default viewer, with platform-specific fallbacks on macOS, Linux, and Windows, and does not require a user-specified output file.
\begin{lstlisting}
